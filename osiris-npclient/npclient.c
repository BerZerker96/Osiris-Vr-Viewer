/*
 * Osiris TrackIR bridge — NPClient.dll / NPClient64.dll
 * =====================================================
 *
 * This is the small "client" DLL that TrackIR / NPClient games load and call
 * to read head-tracking data. It does NOT track anything itself: it reads the
 * FreeTrack 2.0 shared-memory block ("FT_SharedMem") that the Osiris VR Viewer
 * already writes when its "TrackIR Game" toggle is on, and re-presents that
 * pose to the game through the NPClient (TrackIR) ABI (NP_GetData, etc.).
 *
 *   Osiris viewer  --writes-->  FT_SharedMem  --read by-->  this DLL  -->  game
 *
 * The viewer is unchanged; this DLL is the bridge that makes the data the
 * viewer already publishes actually reach NPClient/TrackIR games. Drop it where
 * the game looks for NPClient (see osiris-npclient/README.md).
 *
 * Provenance / licence
 * --------------------
 * Ported essentially verbatim from OpenTrack's `contrib/npclient/npclient.c`
 * (https://github.com/opentrack/opentrack), which is distributed under the
 * permissive ISC licence — NOT the GPL-derived FreeTrack translation. Keeping
 * this clean ISC lineage is deliberate so it can ship alongside Osiris (MIT).
 * The NaturalPoint interface signature reproduced below is required purely for
 * interoperability (so games accept a non-NaturalPoint tracker), exactly as
 * OpenTrack, FreeTrack, FaceTrackNoIR and linuxtrack all do.
 *
 * Build: see build-npclient.bat (one cl.exe command — no Rust, no cargo).
 */
...
/* FreeTrack 2.0 shared-memory block — MUST match the layout the Osiris viewer
 * writes in src/head_output.rs (DataID, CamWidth, CamHeight, then Yaw/Pitch/Roll,
 * then X/Y/Z, raw copies, and 4 point pairs). Rotation is in radians, position
 * in millimetres. Do not reorder. */
...
/* Data block returned to the game by NP_GetData. Field order is fixed by the
 * NPClient ABI: status, frame, cksum, then roll/pitch/yaw, then tx/ty/tz. */
...
/* NPPriv_* are private stubs the original NaturalPoint DLL exports at ordinals
 * 1-7. Games don't call them, but keeping them preserves ABI completeness. */

/* NPPriv_ClientNotify (NPCLIENT.1) */
NP_EXPORT(int) NPPriv_ClientNotify(void)   { dbg_report("stub\n"); return 0; }
/* NPPriv_GetLastError (NPCLIENT.2) */
NP_EXPORT(int) NPPriv_GetLastError(void)   { dbg_report("stub\n"); return 0; }
/* NPPriv_SetData (NPCLIENT.3) */
NP_EXPORT(int) NPPriv_SetData(void)        { dbg_report("stub\n"); return 0; }
/* NPPriv_SetLastError (NPCLIENT.4) */
NP_EXPORT(int) NPPriv_SetLastError(void)   { dbg_report("stub\n"); return 0; }
/* NPPriv_SetParameter (NPCLIENT.5) */
NP_EXPORT(int) NPPriv_SetParameter(void)   { dbg_report("stub\n"); return 0; }
/* NPPriv_SetSignature (NPCLIENT.6) */
NP_EXPORT(int) NPPriv_SetSignature(void)   { dbg_report("stub\n"); return 0; }
/* NPPriv_SetVersion (NPCLIENT.7) */
NP_EXPORT(int) NPPriv_SetVersion(void)     { dbg_report("stub\n"); return 0; }

/* TIR5 requires a checksum calculated over the headpose-data and relayed to the game. */
 ...
 *
 * Reads the FreeTrack block and converts to TrackIR units:
 *   rotation: radians * 16383 / PI
 *   position: millimetres * 16383 / 500
 * then clamps to +/-16383. This is exactly OpenTrack's mapping, so axes line up
 * the same way they do for OpenTrack users (flip in Osiris if one points wrong).
 */
 ...
 *
 * Interop signature: the two halves are XORed at runtime to produce the
 * NaturalPoint Dll/App signatures games may verify. Required for interop only.
 */
